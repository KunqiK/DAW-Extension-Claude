...
- ⏳ **Pending user test:** confirm the two windows feel right (drag/resize the main → editor follows; "▤ Lyrics" toggle). (.exe rebuilt on the two-window build.)

### 2026-06-17 — Phase 3 kickoff: Piapro UI helper (floating panel)
- **Planned w/ user (closed-source reality):** Piapro is a closed MFC/`Afx` VSTi in FL Studio — can't restyle its widgets, only layer external helpers (AHK, like Phase 1). User chose (Q&A): a **floating button panel** that follows the Piapro window; first action = **Play / Stop transport**. Then: "keep working on phase 3 again."
- ✅ **MILESTONE 16 — `hotkeys/PiaproPanel.ahk` v0.1 (built, load-clean):** AHK v2 frameless always-on-top panel (Ina dark palette; an orange flat **▶ Play / Stop** Text-button). A `SetTimer(Track, 250)` docks it to the **top-right of the `Piapro Studio` window** (`WinGetPos`) and follows it; shows **only while FL Studio (`ahk_exe FL64.exe`) is foreground** (or the panel itself) and **hides when Piapro is closed/minimized** (so it never floats over other apps). Click → `WinActivate("Piapro Studio")` + `SendEvent("{Space}")`. Reuses Phase-1 targeting (`SetTitleMatchMode(2)`, title substring). Runs alongside the zoom script.
  - 🔑 **Config vars up top** (the `g_ZoomDelay` style): `TRANSPORT_KEY := "Space"` (best guess — Piapro can't be inspected; **user confirms the real play/stop key**, then it's a 1-line change, like the F8–F11 dance), `HOST_EXE`, `PANEL_W`, dock insets, poll.
  - 🧪 **Validation gotcha:** AHK64 `/validate` returned exit 2 here (switch not honored → treated path oddly); reliable check = run with `/ErrorStdOut` (a load error prints + exits instantly). It ran 168 s with **zero output → loads clean**; killed it. Behaviour (dock position + that Space plays) is user-tested with Piapro open, iteratively.
  - **Built to grow:** action list → next buttons once their Piapro keys/menus are confirmed (phoneme Job, Import/Export, reset zoom).
- ⏳ **Pending user test:** run `PiaproPanel.ahk` with Piapro open → does the panel dock top-right + follow? does **Play/Stop** work (if not, which key?)? is the dock spot good?
